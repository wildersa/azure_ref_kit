...
def test_module_terraform_alignment():
    ...
    tf_variables = [
        list(v.keys())[0].strip('"') for v in variables_tf.get("variable", [])
    ]
    ...
    required_inputs = [
        "container_image",
        "use_managed_identity_for_registry",
        "listen_port",
    ]
    required_outputs = [
        "webapp_name",
    ...
    for req_in in required_inputs:
        assert (
            req_in in module_inputs
        ), f"Required input '{req_in}' missing from module.yaml"
        assert (
            req_in in tf_variables
        ), f"Required input '{req_in}' missing from variables.tf"

    for req_out in required_outputs:
        assert (
            req_out in module_outputs
        ), f"Required output '{req_out}' missing from module.yaml"
        assert (
            req_out in tf_outputs
        ), f"Required output '{req_out}' missing from outputs.tf"
...
def test_managed_identity_default_false():
    ...
    var_config = next(
        (
            ...
            if list(v.keys())[0].strip('"') == "use_managed_identity_for_registry"
        ),
        None,
    )

    assert (
        var_config is not None
    ), "use_managed_identity_for_registry variable missing from variables.tf"
    ...
    ), "use_managed_identity_for_registry must default to false"
...
def test_module_yaml_exposed_inputs_are_backed_by_tf():
    ...
    for module_input in module_inputs:
        assert (
            module_input in tf_variables
        ), f"Input '{module_input}' in module.yaml is not backed by a Terraform variable"
...
def test_module_yaml_exposed_outputs_are_backed_by_tf():
    ...
    for module_output in module_outputs:
        assert (
            module_output in tf_outputs
        ), f"Output '{module_output}' in module.yaml is not backed by a Terraform output"
